...
const AmbulanceCreate: React.FC = () => {
  const [data, setData] = useState({
    facility_id: '',
    vehicle_number: '',
    license_plate: '',
    type: '' as AmbulanceType,
    make_model: '',
    year_of_manufacture: '',
    equipment_inventory: [] as string[],
    capacity: '',
    gps_device_info: '',
    insurance_expiry: '',
    license_expiry: '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setProcessing(true);
    setErrors({});
    axios.post('/api/v1/ambulances', data)
      .then(() => {
        notification.success({ message: 'Ambulance created successfully' });
      ...
      .catch(err => {
        if (err.response && err.response.status === 422 && err.response.data.errors) {
          ...
          notification.error({ message: 'Error', description: 'Failed to create ambulance.' });
        }
  ...
  return (
    <AppLayout user={auth.user}>
      <Head title="Create Ambulance" />
      <div className="py-12">
        <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <Card>
            <CardHeader>
              <CardTitle>Create New Ambulance</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex justify-end pt-4">
                    <Button type="submit" disabled={processing}>Create Ambulance</Button>
                </div>
              </form>
            </CardContent>
          </Card>
